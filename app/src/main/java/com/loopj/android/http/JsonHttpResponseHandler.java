...
public class JsonHttpResponseHandler extends TextHttpResponseHandler {
    ...
    public JsonHttpResponseHandler(FragmentActivity fa) {
        super(fa);
    }
    ...
    boolean notshowErrormessage;

    public void onSuccessNotShowMessage(boolean notshowErrormessage) {
        this.notshowErrormessage = notshowErrormessage;
    }
    ...
    public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
        ...
        try {
            ...
            if (response != null && response.has("boolen") && response.has("logined")) {
                String boolen = response.getString("boolen");
                if (boolen.equals("1")) {
                } else {
                    logined = response.getString("logined");
                    if (logined.trim().equals("0")) {
                        MemorySave.MS.mIsLogin = false;
//            	  		 MemorySave.MS.mIsLoginMark=false;
                        String nameStr = mFa.getIntent().getStringExtra("goClass");
                        GoLoginUtil.ToLoginActivityForResultBase((TRJActivity) mFa, Constants.REQUEST_CODE, nameStr != null ? nameStr : "");
                    } else {
                        String message = response.getString("message");
                        if (notshowErrormessage) return;
                        if (strm.indexOf(message) == -1) ToastUtil.showToast(mFa, message);
                    }
                }
            } else if (response.has("message") && response.has("boolen")) {
                String message = response.getString("message");
                String boolen = response.getString("boolen");
                if (notshowErrormessage) return;
                if (!boolen.equals("1") && message != null && !message.trim().equals("")) {
                    if (strm.indexOf(message) == -1 && message.indexOf("余额不足") == -1)
                        ToastUtil.showToast(mFa, message);
                }
            }
