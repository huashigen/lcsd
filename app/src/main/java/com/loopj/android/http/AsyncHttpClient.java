...
public class AsyncHttpClient {
    ...
    public AsyncHttpClient(SchemeRegistry schemeRegistry) {
        ...
        HttpProtocolParams.setUserAgent(httpParams, "tourongjia(Android 4.0)");
        ...
        try {
            httpClient = MySSLSocketFactory.getNewHttpClient(KeyStore.getInstance(KeyStore.getDefaultType()), httpParams, schemeRegistry);
        } catch (KeyStoreException e) {
            e.printStackTrace();
            httpClient = new DefaultHttpClient(cm, httpParams);
        }// new DefaultHttpClient(cm, httpParams);//
        httpClient.getParams().setParameter(ClientPNames.ALLOW_CIRCULAR_REDIRECTS, false);//待考证这句话的影响
