 ...
 */
package com.lcshidai.lc.widget.xlvfresh;
...
public class XListViewNew extends ListView implements OnScrollListener {
	...
	// -- 底部的View
	private XListViewFooterNew mFooterView;
	...
	private void initWithContext(Context context) {
		...
		// 初始化底部的View
		mFooterView = new XListViewFooterNew(context);
	...
	public void setPullLoadEnable(boolean enable) {
		...
		if (!mEnablePullLoad) {
			...
			mPullLoading = false;
			mFooterView.show();// 显示
			mFooterView.setState(XListViewFooterNew.STATE_NORMAL);
...
//	}
	...
	private void updateFooterHeight(float delta) {
		...
		if (mEnablePullLoad && !mPullLoading) {
			if (height > PULL_LOAD_MORE_DELTA) { // 高度足以调用加载更多
				mFooterView.setState(XListViewFooterNew.STATE_READY);
			} else {
				mFooterView.setState(XListViewFooterNew.STATE_NORMAL);
			}
		}
	...
	private void startLoadMore() {
		mPullLoading = true;
		mFooterView.setState(XListViewFooterNew.STATE_LOADING);
		if (mListViewListener != null) {
	...
	void showTime(){
		if(lastktime==null){
			mHeaderTimeView.setText("从未");
		}else{
			Date nowDate=new Date();
			if(nowDate.getTime()-lastktime.getTime()<60000){
				mHeaderTimeView.setText("刚刚");
			}else{
				mHeaderTimeView.setText(sd.format(lastktime));
			}
		}
	}
}
