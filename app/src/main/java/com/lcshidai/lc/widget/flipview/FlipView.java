...
import com.lcshidai.lc.widget.flipview.Recycler.Scrap;
...
public class FlipView extends FrameLayout {
	...
	private int ACTION_DURATION = 400;// in ms
	private static final int PEAK_ANIM_DURATION = 1000;// in ms
	...
	private void init() {
		...
		mScroller = new OverScroller(context, flipInterpolator);
		mTouchSlop = configuration.getScaledPagingTouchSlop();
		mMinimumVelocity = configuration.getScaledMinimumFlingVelocity();
		mMaximumVelocity = configuration.getScaledMaximumFlingVelocity();

		mShadowPaint.setColor(Color.WHITE);// #E8E8E8
		mShadowPaint.setStyle(Style.FILL);
		mShadePaint.setColor(Color.rgb(0xe8, 0xe8, 0xe8));// #E8E8E8
		mShadePaint.setStyle(Style.FILL);
		mShinePaint.setColor(Color.rgb(0xe8, 0xe8, 0xe8));// #E8E8E8
	...
	private void hideOtherPages(Page p) {
		if (mPreviousPage != p && mPreviousPage.valid
				&& mPreviousPage.v.getVisibility() != GONE) {
			mPreviousPage.v.setVisibility(GONE);
		}
		if (mCurrentPage != p && mCurrentPage.valid
				&& mCurrentPage.v.getVisibility() != GONE) {
			mCurrentPage.v.setVisibility(GONE);
		}
		if (mNextPage != p && mNextPage.valid
				&& mNextPage.v.getVisibility() != GONE) {
	...
	private void setDrawWithLayer(View v, boolean drawWithLayer) {
		if (Build.VERSION.SDK_INT >= 11) {
			...
			if (isHardwareAccelerated()) {
				...
				} else if (v.getLayerType() != LAYER_TYPE_NONE
						&& !drawWithLayer) {
	...
	public void smoothFlipTo(int page) {
		...
		endFlip();
		mScroller.startScroll(0, start, 0, delta, ACTION_DURATION);
		invalidate();
	}
	...
	public void smoothFlipByAll(int delta) {
		setAdapter(new FlipAdapter(mContext, 36001, 10, false));
		flipAll();
		int page = mCurrentPageIndex + delta;
		final int start = (int) mFlipDistance;
		final int deltas = page * FLIP_DISTANCE_PER_PAGE - start;

		endFlip();
		// if(page==0){
		// }else{
		// mScroller.startScroll(0, start, 0, delta, 9000);
		// }
		// mScroller.startScroll(0, start, 0, delta, 1000*Math.abs((int)delta));
		mScroller.startScroll(0, start, 0, deltas, ACTION_DURATION);
		invalidate();
	}
