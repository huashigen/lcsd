	...
	public void onClick(View v) {
		...
		if (plat != null) {
			...
			if(!name.equals("ShortMessage")){
				reqData.put("text", reqData.get("desn"));
			}
...
//				if("QQ".equals(name)){
//					HashMap<String, Object> tempReqData = new HashMap<String, Object>();
//					tempReqData = (HashMap<String, Object>) reqData.clone();
//					if(null != tempReqData.get("imagePath") && !"".equals(tempReqData.get("imagePath"))){
//						if(((String)tempReqData.get("text")).length() > 10){
//							String text = ((String)tempReqData.get("text")).substring(0, 10) + "…";
//							tempReqData.remove("text");
//							tempReqData.put("text", text);
//						}
//					}
//					shareData.put(plat, tempReqData);
//				}else{
//					shareData.put(plat, reqData);
//				}
				shareData.put(plat, reqData);
	...
	private static class GridView extends LinearLayout {
		...
		private void init() {
			...
			int dip_10 = cn.sharesdk.framework.utils.R.dipToPx(getContext(), 10);
			int dp_20 = cn.sharesdk.framework.utils.R.dipToPx(getContext(), 20);
			
			setPadding(dip_10, 0, dip_10, dp_20);
			setOrientation(VERTICAL);
			...
			LayoutParams lp = new LayoutParams(
					(int)(scrW - dip_12 - dip_12 - dip_10 - dip_10), LayoutParams.MATCH_PARENT);
			lp.weight = 1;
			for (int i = 0; i < lines; i++) {
				...
				llLine.setPadding(0, dip_10, 0, 0);
				llLine.setGravity(Gravity.CENTER_HORIZONTAL);
				addView(llLine);
